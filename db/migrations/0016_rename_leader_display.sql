-- Renames expeditions.leader to leader_display to clarify it is decorative data
-- synced from Google Drive, not a verified system user.
-- The verified leader is represented by expedition_members (role='leader', status='approved').

alter table public.expeditions
  rename column leader to leader_display;

-- Re-create list_expeditions RPC with updated column name in WHERE clause
...
values ('0016_rename_leader_display')
