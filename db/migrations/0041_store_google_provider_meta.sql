...
-- Adds email column pre-filled from Google.
-- Updates handle_new_user to persist name, avatar, email, and raw metadata.

alter table public.user_profiles
  add column if not exists email        text,
  add column if not exists provider_meta jsonb;

-- Rebuild trigger function to store name, avatar, email, and raw provider metadata.
...
begin
  ...
  values (new.id, 'newcomer', _name, _avatar_url, new.email, current_date, new.raw_user_meta_data)
